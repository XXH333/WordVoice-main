...
def parse_embedding(data, normalize, mode='train'):
    ...
    for sample in data:
        ...
        elif 'utt_embedding' in sample and 'spk_embedding' not in sample:
            sample['utt_embedding'] = torch.tensor(sample['utt_embedding'], dtype=torch.float32)
            sample['spk_embedding'] = sample['utt_embedding']
        else:
            sample['utt_embedding'] = torch.tensor(sample['utt_embedding'], dtype=torch.float32)
...
def tokenize(data, get_tokenizer, allowed_special, mode='train'):
    ...
    for sample in data:
        ...
        if 'instruct' in sample:
            ...
            sample['instruct'] = "You are a helpful assistant.<|endofprompt|>"
            sample['instruct_token'] = tokenizer.encode(sample['instruct'], allowed_special=allowed_special)
        if 'words' in sample:
            sample['words'] = [tokenizer.encode(word, allowed_special=allowed_special)[0] for word in sample['words']] # [tokenizer.encode('s', allowed_special=allowed_special)[0] for word in sample['words']]
        yield sample

def word_process(data, boundary_list=['b0', 'b1', 'b2', 'b3', 'b4'], tone_list = ['flat', 'rise', 'rrise', 'fall', 'ffall' 'peak', 'valley'], mode='train'):
    for sample in data:
        assert 'start' in sample
        start_list = []
        end_list = []
        max_len = len(sample['speech_token'])
        ...
        for f in sample['f0']:
            if math.isnan(f):
                ...
                is_valid = False
                break
        for e in sample['energy']:
            if math.isnan(e):
                sample['energy'] = 0
                is_valid = False
                break
        if is_valid is False:
            logging.warning('Invalid f0 or energy in sample {}.'.format(sample['utt']))
            continue

        is_valid = True
        last_end = 0
        for i in range(len(sample['words'])):
            start = int(sample['start'][i]*25 + 0.6)
            start = max(start, last_end)
            end = int(sample['end'][i]*25 + 0.6)
            end = max(start+1, end)
            end = min(max_len, end)
            if start >= end:
                logging.warning('Invalid start and end in sample {}, start {}, end {}'.format(sample['utt'], start, end))
                is_valid = False
                break
            last_end = end
        ...
        sample['start'] = start_list
        sample['end'] = end_list
        sample['boundary'] = [
            boundary_list.index(boundary) if boundary in boundary_list else len(boundary_list)
            for boundary in sample['boundary']
        ]
        sample['tone'] = [
            tone_list.index(tone) if tone in tone_list else len(tone_list)
            for tone in sample['tone']
        ]
        yield sample
...
def wordvoice_padding(data, use_spk_embedding, mode='train', gan=False, dpo=False):
    """ Padding the data into training data
    ...
    for sample in data:
        ...
        batch['utts'] = [sample[i]['utt'] for i in order]
        batch['text'] = [sample[i]['text'] for i in order]
        words = [sample[i]['words'] for i in order]
        batch['word_len'] = torch.tensor([len(i) for i in words], dtype=torch.int32)
        batch['words'] = pad_sequence([torch.tensor(i) for i in words], batch_first=True, padding_value=0)
        batch['start'] = [sample[i]['start'] for i in order]
        batch['end'] = [sample[i]['end'] for i in order]
        batch['boundary'] = [sample[i]['boundary'] for i in order]
        batch['tone'] = [sample[i]['tone'] for i in order]
        batch['f0'] = [sample[i]['f0'] for i in order]
        batch['energy'] = [sample[i]['energy'] for i in order]
