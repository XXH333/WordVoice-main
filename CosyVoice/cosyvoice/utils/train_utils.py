...
def init_distributed(args):
    ...
    timeout_duration = datetime.timedelta(seconds=3600)
    logging.info('training on multiple gpus, this gpu {}'.format(local_rank) +
                 ', rank {}, world_size {}'.format(rank, world_size))
    if args.train_engine == 'torch_ddp':
        torch.cuda.set_device(local_rank)
        dist.init_process_group(args.dist_backend, timeout=timeout_duration)
    else:
        deepspeed.init_distributed(dist_backend=args.dist_backend, timeout=timeout_duration)
