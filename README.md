...
本仓库包含了 WordVoice 模型的**训练**与**推理**代码。基于我们开源的 [WordVoice-5A 数据集](#) 及语言学 Pipeline，本模型实现了对语音生成过程中**五个声学维度**的字级别精准解耦控制。
...
运行以下脚本，自动下载 WordVoice 预训练模型权重及相关依赖模型（如 CosyVoice3、MMS-FA 等）：
```bash
...
```
*在运行训练脚本前，请确保您已经使用我们的 [WordVoice Data Pipeline](#) 完成了数据的预处理与字级声学特征提取，并在脚本中正确配置了数据路径与超参数。*
